...
$("#virtualparking-add-form").validate({
       submitHandler: function(form) {
		...
		var searchData={areaId:areaId,parkingLotsId:lotsId,parkingFloor:parkingFloor,parkingMax:$("#parking-number").val(),index:$("#start-number").val(),type:"1"};
			 $.ajax({
             type: "post",
             ...
             success: function(data){
				 ...
				 if(dataCode==200){
					 alert("数据添加成功！");
					 getParking(areaId,lotsId,parkingFloor,"v_bus",areaName);//获取标准车位
					 }
				else{
					alert(data.msg);
					}
	...
	$("#parking-floor").change(function(){
		 ...
		 getParking(areaId,lotsId,parkingFloor,"v_bus",areaName);//获取物业车位
		});
	...
	$(document).on("click",".page .pageone",function(){
			...
			console.log(rePageIndex);
			 ...
			 if(!rePageIndex){
			 	...
			 	getParking(areaId,lotsId,parkingFloor,"v_bus",areaName,rePageIndex,pageSize);
			 }
