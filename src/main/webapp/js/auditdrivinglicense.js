		...
		html+='<li>'+
				'<div class="photo photo-checkbox fl col-6">'+
					'<div class="checkinput-box checkinput-box-sfsh">'+
	              	 /*'<span class="check-title">选中审核：</span>'+*/
	                 '<form>'+
				  ...
				  $('#table-a-list').html(""); 
				   if(data.data.total==0){
					...
					if(data.code==200){
                 	...
                 	+'<a href="javascript:void(0);"><img src='+picurl[0]+' class="img-small"/>'
             ...
             success: function(data){
				  ...
				  console.log(data.data);
					if(data.code==200){
                 	...
                 	html += '<tr class='+trClass+'><td class="userName ta-l" userid='+comment.user_id+'>'+comment.user_name+'</td>'
                 	+'<td>'+comment.mobile+'</td>'
                 	+'<td class="xsz-img" picurl='+picurl[0]+' vehicleId='+comment.vehicle_id+' picurlarr=\''+picurl+'\'>'
                 	+'<a href="javascript:void(0);"><img src='+picurl[0]+' class="img-small"/>'
                 	+'<img src='+picurl[0]+' class="img-big"/></a></td>'
                 	+'<td>'+status+'</td>'
               +'<td class="change operation-btn ta-r">'+opr+'</td></tr>';
...
function getAuditPcInfo(vehicleId){
	var searchData={userName:"",status:"NEW",phone:"",vehicleId:vehicleId};
	 $.ajax({
             type: "get",
             url: "/cms/vehicle/getVehiclePics",
             data: searchData,
...
function auditPhoneDrivingLic(){
	...
	var urlStr="";
	var sfsh="";
	var flag=0;//标记勾选了的正本数量
	picLi.each(function(i,data) {//循环取得图片
		$this=$(this);
		sfsh=$this.find(".sfsh").prop("checked");
		console.log(sfsh);
		if(sfsh==true){
...
function auditPcDrivingLic(){
	...
	var comment=$("#pcdisagree").val();
	 var dataJson={vehicleId:vehicleId,status:status,urls:urlStr,comment:comment};
